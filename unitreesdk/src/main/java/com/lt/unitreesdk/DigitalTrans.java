...
public class DigitalTrans {
    ...
    /**
     * 十六进制字符串转为int组,每两个十六进制字符转为一个int
     *
     * @param hex 十六进制字符串
     * @return int 转换结果
     */
...
//        int[] ints = new int[hexs.length];
//        for (int i = 0, k = hexs.length; i < k; i++) {
//            ints[i] = Integer.valueOf(hexs[i], 16);
//        }
//        return ints;
//    }
...
//            throw new IllegalArgumentException();
        ...
        for (int i = 0, j = 0, l = hex.length(); i < l; i++, j++) {
            String swap = "" + arr[i++] + arr[i];
            ints[j] = Integer.parseInt(swap, 16) & 0xFF;
        }
        return ints;
    }
